# 🥗 Calculadora de Dieta

Calculadora de dieta completa e gratuita para calcular TMB, gasto energético e macronutrientes personalizados.

## Funcionalidades

- ✅ Cálculo de Taxa Metabólica Basal (TMB)
- ✅ Cálculo de Gasto Energético Total (GET)
- ✅ Cálculo de Macronutrientes (Proteína, Gordura, Carboidrato)
- ✅ Personalização de deficit/superavit calórico
- ✅ Personalização de macronutrientes por kg
- ✅ Interface moderna e responsiva
- ✅ 100% gratuito e sem anúncios

## Deploy

Este projeto está hospedado no Cloudflare Pages.

## Tecnologias

- HTML5
- CSS3
- JavaScript (Vanilla)
- SVG

## Como usar

1. Preencha seus dados (sexo, peso, altura, idade)
2. Selecione seu nível de atividade física
3. Escolha seu objetivo (perder, manter ou ganhar peso)
4. Ajuste os macronutrientes conforme sua preferência
5. Clique em "Calcular Dieta"

## License

MIT
